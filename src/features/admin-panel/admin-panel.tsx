...
const AdminPanel: FC = () => {
  ...
  const getPathValue = (path: string) =>
    Object.keys(tabPaths).find((key) => tabPaths[key] === path) || "1";

  const [value, setValue] = useState(getPathValue(location.pathname));

  const handleChange = (_: SyntheticEvent, newValue: string) => {
    setValue(newValue);
    const newPath = tabPaths[newValue] || "/admin-panel";
    navigate(newPath);
  };
