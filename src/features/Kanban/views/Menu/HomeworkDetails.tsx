...
import LectureHomeworkContainer from "../../containers/LectureHomework";
...
const HomeworkDetails: React.FC<IHomeworkDetail> = ({ card, onClose }) => {
  ...
  return (
    <Box sx={style.menu}>
      ...
      <LectureHomeworkContainer lectureId={card.lecture?.id!} />
      <Homework dataUserId={card.student?.id!} lectureId={card.lecture?.id!} />
    </Box>
  );
};
